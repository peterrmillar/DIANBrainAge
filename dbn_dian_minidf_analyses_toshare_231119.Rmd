...
```{r}
#load packages
pacman::p_load(tidyverse, corrplot, psych, data.table, pander, clean, Metrics, ggpubr, gtsummary, neuroCombat, longCombat, Metrics, DescTools, viridis, dutchmasters, ggsci, lme4, nlme, lmerTest, multilevelTools, splines, AICcmodavg, effects, mgcv, gamm4, ggthemes, effectsize, ez, heplots, irr, gridExtra, mediation, caret)
...
data_long$Race = recode_factor(as.factor(data_long$Race)
                          , "Aboriginal" = "Aboriginal Australian or Torres Strait Islander"
                          , "Aboriginal Australian" = "Aboriginal Australian or Torres Strait Islander"
                          , "Australian aboriginal" = "Aboriginal Australian or Torres Strait Islander"
                          , "Arab" = "Arab"
                          , "Asian" = "Asian"
                          , "Black or African American" = "Black or African American"
                          , "Croatian" = "Unknown"
...
scatter_fit_non = ggplot(data_non, aes(x = Age, y = Pred_Age_DBN)) +
  geom_point(color = "blue") +
  labs(title="DBN-Predicted Brain Age (Unharmonized)",x="True Age", y = "Predicted Age") +
  geom_smooth(col = 'blue', method = lm, linetype = 'longdash') + 
  ...
  theme_bw(base_size = 15) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.12)-0, color = "blue", label = paste("italic(r) == ", round(cor(data_non$Age, data_non$Pred_Age_DBN), 2)),
  parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.08)-0, color = "blue", label = paste("italic(R) ^ 2 == ", round(R2(pred = data_non$Pred_Age_DBN, obs = data_non$Age, formula = "traditional"), 2))
  , parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.04)-0, color = "blue", label = paste("italic(MAE) == ", round(MAE(data_non$Age, data_non$Pred_Age_DBN), 2)),
  parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.00)-0, color = "blue", label = paste("italic(RMSE) == ", round(RMSE(data_non$Age, data_non$Pred_Age_DBN), 2)),
  parse = TRUE) +
  theme(
...
ggsave(paste(out_path, '/model_fit/dian_df15_fit_non_', ver_date, '.png', sep = ""), width = 6, height = 6, units = "in", dpi = 320)
...
scatter_fit_non_harmonized = ggplot(data_non, aes(x = Age, y = Pred_Age_Choice)) +
  ...
  parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.08)-4, color = "blue", label = paste("italic(R) ^ 2 == ", round(R2(pred = data_non$Pred_Age_Choice, obs = data_non$Age, formula = "traditional"), 2))
  , parse = TRUE) +
...
test_r_harm = cor(data_non$Pred_Age_Choice, data_non$Age)
test_r2_harm = R2(pred = data_non$Pred_Age_Choice, obs = data_non$Age, formula = "traditional")
...
scatter_fit_non = ggplot(data_non, aes(x = Age, y = Pred_Age_DBN_delange)) +
  geom_point(color = "blue") +
  ...
  ylim(min(c(data$Age, yscale)), max(c(data$Age, yscale))) + 
  theme_bw(base_size = 15) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.12)-0, color = "blue", label = paste("italic(r) == ", round(cor(data_non$Age, data_non$Pred_Age_DBN_delange), 2)),
  parse = TRUE) +
    annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.08)-0, color = "blue", label = paste("italic(R) ^ 2 == ", round(R2(pred = data_non$Pred_Age_DBN_delange, obs = data_non$Age, formula = "traditional"), 2))
  , parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.04)-0, color = "blue", label = paste("italic(MAE) == ", round(MAE(data_non$Age, data_non$Pred_Age_DBN_delange), 2)),
  parse = TRUE) +
  annotate("text", x = max(yscale)-((max(yscale)-min(yscale))*0.1)+4, y = min(yscale)+((max(yscale)-min(yscale))*0.00)-0, color = "blue", label = paste("italic(RMSE) == ", round(RMSE(data_non$Age, data_non$Pred_Age_DBN_delange), 2)),
  parse = TRUE) +
  theme(
    plot.title = element_text(hjust = 0.5)#, legend.title = element_blank()
    , legend.key = element_blank(), legend.background=element_blank()
...
scatter_dif_mutationsym_performance = ggplot(data, aes(x = Age, y = Pred_Age_Choice)) +
  ...
  annotate("text", x = 76, y = min(yscale)+((max(yscale)-min(yscale))*0.08), color = "blue", label = paste("italic(R) ^ 2 == ", round(R2(pred = data_non$Pred_Age_Choice, obs = data_non$Age, formula = "traditional"), 2)), parse = TRUE) +
...
test_r_non = cor(data_non$Pred_Age_Choice, data_non$Age)
test_r2_non = R2(pred = data_non$Pred_Age_Choice, obs = data_non$Age, formula = "traditional")
test_mae_non = mae(data_non$Pred_Age_Choice, data_non$Age)
test_rmse_non = rmse(data_non$Pred_Age_Choice, data_non$Age)

test_r_car = cor(data_car$Pred_Age_Choice, data_car$Age)
test_r2_car = R2(pred = data_car$Pred_Age_Choice, obs = data_car$Age, formula = "traditional")
...
scatter_dif_mutationsym_performance_agecorrected = ggplot(data, aes(x = Age, y = Pred_Age_DBN_delange)) +
  ...
  annotate("text", x = 76, y = min(yscale)+((max(yscale)-min(yscale))*0.08), color = "blue", label = paste("italic(R) ^ 2 == ", round(R2(pred = data_non$Pred_Age_DBN_delange, obs = data_non$Age, formula = "traditional"), 2))
  , parse = TRUE) +
...
test_r_non = cor(data_non$Pred_Age_DBN_delange, data_non$Age)
test_r2_non = R2(pred = data_non$Pred_Age_DBN_delange, obs = data_non$Age, formula = "traditional")
test_mae_non = mae(data_non$Pred_Age_DBN_delange, data_non$Age)
test_rmse_non = rmse(data_non$Pred_Age_DBN_delange, data_non$Age)

test_r_car = cor(data_car$Pred_Age_DBN_delange, data_car$Age)
test_r2_car = R2(pred = data_car$Pred_Age_DBN_delange, obs = data_car$Age, formula = "traditional")
...
###GAMM
```{r}
#redefine mutation status as an ordered factor (for GAM interaction)
...
#from Julie - function to calculate CIs
...
#find earliest point of non-overlapping CIs
